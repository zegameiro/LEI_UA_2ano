...
{
  private:
    const static int max_size = 99;   // Static -> quer dizer que é uma variável comum a todos os objetos da classe
  ...
  public:
    ...
    person(const char *name = "",const char *address = "", const char *phonenumber = "");
...
person::~person(void)  // Limpa o espaço alocado na memória
{ // destructor
  if(debug != 0)
...
int main(void)
{
  person p = person("John Doe","no address","914673891"); // create a new object and then copy it to p
